...
10. ~~**Adresse postale exacte** du bureau à Tanger~~ — **RÉSOLU le 2026-08-23** : adresse confirmée par le client (revue copy annotée, Mod 2) = « 184 Borj Khalij, Tanger », propagée dans `lib/config.ts` (`office.address`). **Reste ouvert : coordonnées GPS.** `lat`/`lng` et `mapImage` sont encore le placeholder Tanger-centre (35.7595, -5.834) faute de coordonnées vérifiées pour cette adresse précise — voir `TODO(coordinates)` dans `lib/config.ts`. Ne pas géocoder à l'aveugle : demander au client ou vérifier sur place, puis régénérer la tuile OSM. Le footer et la page Contact utilisent `site.address` (général, « Tanger, Maroc »), distinct de `site.office.address` — non concerné par cette résolution, hors périmètre de cette revue.
